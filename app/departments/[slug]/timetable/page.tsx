    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          {Object.keys(groupedTimetables).sort().map((semester) => (
            <div key={semester} className="mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Semester {semester}</h2>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {groupedTimetables[semester].map((tt: any) => (
                  <div key={tt.id} className="bg-white border-2 border-gray-200 rounded-xl p-6 hover:shadow-xl hover:border-orange-600 transition">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-12 h-12 bg-orange-100 rounded-lg flex items-center justify-center">
                        <Calendar className="w-6 h-6 text-orange-600" />
                      </div>
                      <div>
                        <h3 className="font-bold text-gray-800">{tt.class_name}</h3>
                        <p className="text-sm text-gray-600">Semester {tt.semester}</p>
                      </div>
                    </div>
                    <p className="text-sm text-gray-600 mb-4">Academic Year: {tt.academic_year}</p>
                    <a
                      href={tt.file_url}
