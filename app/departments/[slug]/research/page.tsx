  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([depts, researchData]) => {
        ...
        setDepartment(dept);
        if (dept) {
          const deptResearch = researchData.filter((r: any) => r.department_id === dept.id);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="space-y-6">
            {research.map((project) => (
              <div key={project.id} className="bg-white border-2 border-gray-200 rounded-xl p-8 hover:shadow-xl transition">
                <h3 className="text-xl font-bold text-gray-800 mb-2">{project.title}</h3>
                <p className="text-gray-600 mb-4">{project.description}</p>
                <div className="grid md:grid-cols-4 gap-4">
                  <div>
                    <p className="text-sm font-semibold text-gray-800">PI: {project.pi_name}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">{project.funding_agency}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">{project.start_year} - {project.end_year}</p>
                  </div>
                  <div>
                    <p className="text-sm font-bold text-green-600">{project.funding_amount}</p>
                  </div>
                </div>
              </div>
            ))}
